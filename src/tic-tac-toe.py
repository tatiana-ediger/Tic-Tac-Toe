...
class TicTacToeAction:
    ...
    def __str__(self):
        return f"{self.player} plays at ({self.row}, {self.col})"
...
def minimax_decision(state: TicTacToeState):
    ...
    # return max(map(lambda action: min_value(play(state, action), possible_actions(state))))
    for action in possible_actions(state):
        utility = min_value(play(state, action))
        if utility > max_so_far[1]:
            max_so_far = [action, utility]
...
# ---- TESTING PLAY
assert play(empty_game_state, TicTacToeAction(0,0,'X')) == TicTacToeState([[-1, -1, -1], [-1, -1, -1], [-1, -1, -1]], 'O')
